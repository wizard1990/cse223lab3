...
func (self *attClient) Set(kv *trib.KeyValue, succ *bool) error {
	...
	clk, resL, _ := FindLargestClock(&res0, &res1, &res2)
	...
	for _, ele := range resL.L {
		oldclk, _ := SplitClock(ele)
		if oldclk < n {
			for i := 0; i < 3; i++ {
				go func(i int) {
					t := 0
					self.client[i].ListRemove(&trib.KeyValue{genPrefix(self.bin) + colon.Escape(kv.Key) + "::KV", ele}, &t)
				}(i)
			}
		}
	}
